// SPDX-FileCopyrightText: (C) 2025 pascalkuthe <pascalkuthe@pm.me>
    ...
    T: Display,
{
    ...
    pub fn new(input: &'a InternedInput<T>, use_color: bool) -> Self {
        Self {
            before_hunk_start: 0,
            after_hunk_start: 0,
            before_hunk_len: 0,
    ...
    T: Display,
{
    ...
    fn print_tokens(&mut self, tokens: &[Token], prefix: char) {
        for &token in tokens {
            ...
            if self.color {
                match prefix {
                    '-' => write!(self.buffer, "\x1b[31m{line}\x1b[0m").unwrap(),
                    '+' => write!(self.buffer, "\x1b[32m{line}\x1b[0m").unwrap(),
                    _ => self.buffer.write_str(&line).unwrap(),
                }
            } else {
                self.buffer.write_str(&line).unwrap();
            }
        }
    }
